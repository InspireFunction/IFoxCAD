...
public class IMEControl
{
    ...
    static IMEControl()
    {
        ...
        string? lines = null;
        _ftFile = Path.Combine(Settings.MyDir, nameof(Gstar_IMEFilter) + nameof(ExceptCmds) + ".ft");
        if (File.Exists(_ftFile))
            lines = File.ReadAllText(_ftFile, Encoding.UTF8);
        else
            Debugx.Printl("配置文件丢失: " + _ftFile);
        if (lines != null)
        {
    ...
    public static void SaveFt()
    {
        ...
        foreach (var item in ExceptCmds)
            if (!DefaultCmds.Contains(item))
                lst.Add(item);
        if (!lst.Any())
            return;
        var jo = string.Join(",", lst.ToArray());
        File.WriteAllText(_ftFile, jo, Encoding.UTF8);
    }
        ...
        if (Settings.IMEStyle == IMEHookStyle.Process)
        {
            ...
            _hookProc = (nCode, wParam, lParam) => {
                if (nCode >= 0)
                {
                    ...
                    // 高版本cad基本上不能用进程钩子 Gstar_IMEFilterSettings

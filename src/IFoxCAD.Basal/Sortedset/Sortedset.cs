...
namespace System.Collections.Generic
{
    ...
    [DebuggerTypeProxy(nameof(SortedSet<T>))]/*这句改了*/
    ...
    public class SortedSet<T> : ISet<T>, ICollection<T>, ICollection, IReadOnlyCollection<T> {
        ...
        internal virtual bool InOrderTreeWalk(TreeWalkPredicate<T> action, bool reverse)
        {
            ...
            Stack<Node> stack = new(2 * (int)(SortedSet<T>.Log2(Count + 1)));
            Node current = root;
            while (current != null)
            {
                stack.Push(current);
        ...
        public void UnionWith(IEnumerable<T> other)
        {
            ...
            {
                SortedSet<T> dummy = new(s, this.comparer);
        ...
        internal sealed class TreeSubSet : SortedSet<T> {
            ...
            internal override bool InOrderTreeWalk(TreeWalkPredicate<T> action, Boolean reverse)
            {
                ...
                // The maximum height of a red-black tree is 2*lg(n+1).
                // See page 264 of "Introduction to algorithms" by Thomas H. Cormen
                Stack<Node> stack = new(2 * (int)SortedSet<T>.Log2(count + 1)); // this is not exactly right if count is out of date, but the stack can grow
                Node current = root;
                while (current != null)
                {
        ...
        public struct Enumerator : IEnumerator<T>, IEnumerator {
            ...
            static readonly SortedSet<T>.Node dummyNode = new(default);
    ...
    internal class SortedSetEqualityComparer<T> : IEqualityComparer<SortedSet<T>>
    {
        ...
        public int GetHashCode(SortedSet<T> obj)
        {
            ...
            if (obj != null)
            {
                foreach (T t in obj)
                {
                    hashCode ^= (e_comparer.GetHashCode(t) & 0x7FFFFFFF);
                }

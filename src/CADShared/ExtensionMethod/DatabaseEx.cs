...
public static class DatabaseEx
{
        ...
        bool automatic = true, string? saveAsFile = null, bool echoes = true)
    {
        ...
        string? fileMsg;
        var creatFlag = false;
        if (StringHelper.IsNullOrWhiteSpace(saveAsFile))
        {
            fileMsg = db.Filename;
            saveAsFile = fileMsg;
            //creatFlag = true;
        ...
        else
        {
            ...
            // 文件名缺失时
            if (!creatFlag && StringHelper.IsNullOrWhiteSpace(Path.GetFileName(saveAsFile)))
                creatFlag = true;
        }
        ...
        if (saveAsFile != null)
        {
            ...
            if (StringHelper.IsNullOrWhiteSpace(fileNameWith))
                creatFlag = true;
        }
        else
        {
        ...
        if (creatFlag)
        {
            ...
            if (echoes && error)
                System.Windows.Forms.MessageBox.Show($@"错误参数:{fileMsg} 它将保存:{file}", "错误的文件路径");
            saveAsFile = file;
        }
    ...
    private static (bool error, string path) GetOrCreateSaveAsFile(this Database db)
    {
        ...
        if (StringHelper.IsNullOrWhiteSpace(fileExt))
            fileExt = ".dwg";
